...
//
//  Created by Radithya Reddy on 10/14/19.
...
class MealInterfaceController: WKInterfaceController, WCSessionDelegate {
    ...
    var message: [String: String] {
              return ["timer": String(format: "%.1f", totalTime)]
          }
    ...
    @IBAction func didPressStartStop() {
            if startStopButtonStatus {
                ...
                timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(MealInterfaceController.updateTimer), userInfo: nil, repeats: true);
                timerIsRunning = true;
                }
            } else {
